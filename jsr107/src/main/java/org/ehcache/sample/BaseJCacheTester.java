...
public abstract class BaseJCacheTester {
    ...
    protected <K, V> boolean simpleIterationCacheTest(Cache<K, V> myJCache, int numberOfIteration, int numberOfObjectPerIteration, int sleepTimeMillisBetweenIterations, KeyValueGenerator<K, V> keyValueGenerator) throws InterruptedException {
        try {
            LOGGER.info("simpleIterationCacheTest BEGIN");
            if (null != myJCache) {
                //config check to log
                ...
                while (iterationCount < numberOfIteration) {
                    LOGGER.info("simpleIterationCacheTest -------------------------------------------------------------");
                    LOGGER.info("simpleIterationCacheTest - Iteration #{} of {}", iterationCount + 1, numberOfIteration);
                    ...
                    LOGGER.info("simpleIterationCacheTest - Done Iteration #{} of {} in {} micros - total cache ops: {} hits: {} / misses: {}", iterationCount + 1, numberOfIteration, (duration / 1000), opsCount, hitCounts, missCount);
                ...
                LOGGER.info("simpleIterationCacheTest - Successfully executed {} iteration.", iterationCount);
                return true;
            } else {
                LOGGER.error("simpleIterationCacheTest - Cache object is null...doing nothing...");
                return false;
            }
        } finally {
            LOGGER.info("simpleIterationCacheTest DONE");
        }
    }
